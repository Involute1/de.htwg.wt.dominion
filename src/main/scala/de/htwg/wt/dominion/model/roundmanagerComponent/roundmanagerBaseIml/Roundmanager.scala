...
case class Roundmanager(players: List[Player], names: List[String], numberOfPlayers: Int, turn: Int, decks: List[List[Card]],
  ...
  def cellarActionEnd(input: List[Int]): List[Player] = {
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Post("http://localhost:8081/player/discard", (input, this.players(this.playerTurn))))
  ...
  def merchantCheckForSilver(playerList: List[Player]): List[Player] = {
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/checkForFirstSilver", playerList(this.playerTurn)))
  ...
  def removeHandCardAndAddToStacker(input: Int, playerList: List[Player]): Player = {
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Post("http://localhost:8081/player/removeHandCardAddToStacker", (input, playerList(this.playerTurn))))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
    updatedPlayer
  ...
  def buyCard(input: String): List[Player] = {
    ...
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/updateMoney", (money, updatedPlayerList(this.playerTurn))))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
  ...
  def updateMoneyForRoundmanager(playerList: List[Player]): List[Player] = {
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/calculatePlayerMoneyForBuy", playerList(this.playerTurn)))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
    playerList.patch(this.playerTurn, Seq(updatedPlayer), 1)
  ...
  def addToPlayerMoney(moneyToAdd: Int, playerList: List[Player]): List[Player] = {
    ...
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/updateMoney", (updatedMoney, playerList(this.playerTurn))))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
    playerList.patch(this.playerTurn, Seq(updatedPlayer), 1)
  ...
  def addToPlayerActions(actionsToAdd: Int, playerList: List[Player]): List[Player] = {
    ...
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/updateActions", (updateActions, playerList(this.playerTurn))))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
    playerList.patch(this.playerTurn, Seq(updatedPlayer), 1)
  ...
  def addToPlayerBuys(buysToAdd: Int, playerList: List[Player]): List[Player] = {
    ...
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/updateBuys", (updatedBuys, playerList(this.playerTurn))))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
    playerList.patch(this.playerTurn, Seq(updatedPlayer), 1)
  ...
  def addToTrash(input: Int): (List[Card], List[Player]) = {
    ...
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/trashHandCard", (input, this.players(this.playerTurn))))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
  ...
  def drawXAmountOfCards(cardDrawAmount: Int, player: Player): List[Player] = {
    val updatedPlayer: Player = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/updateHand", (cardDrawAmount, player)))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
  ...
  def checkIfHandContainsTreasure(): Boolean = {
    val handContainsTreasure: Boolean = {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/checkForTreasure", this.players(this.playerTurn)))
  ...
  private def nextPlayer(): Roundmanager = {
    if (this.emptyDeckCount == 3) {
      ...
      val removeHandPlayer: Player = {
        ...
        val response = Http().singleRequest(Get("http://localhost:8081/player/removeCompleteHand", (this.players(this.playerTurn), this.players(this.playerTurn).handCards.size - 1)))
        val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
        Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
      }
      val handCardPlayer: Player = {
        //val response = Http().singleRequest(Get("http://player:8081/player/updateHand", (5, removeHandPlayer)))
        val response = Http().singleRequest(Get("http://localhost:8081/player/updateHand", (5, removeHandPlayer)))
        val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
        Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
      }
  ...
  private def calculateScore(): List[(String, Int)] = {
    val updatedPlayerList: List[Player] = for (player <- this.players) yield {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/moveAllCardsToDeckForScore", player))
      val jsonFuture = response.flatMap(r => Unmarshal(r.entity).to[Player])
      Await.result(jsonFuture, Duration(1, TimeUnit.SECONDS))
    }
    val scoreList: List[Int] = for (player <- updatedPlayerList) yield {
      ...
      val response = Http().singleRequest(Get("http://localhost:8081/player/calculateScore", player))
